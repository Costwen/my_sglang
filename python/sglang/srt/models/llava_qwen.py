...
class LlavaVidQwenForCausalLM(nn.Module):
    def __init__(
        self,
        config: LlavaConfig,
    ...
    def encode_images(self, pixel_values: torch.Tensor) -> torch.Tensor:
        image_outputs = self.vision_tower(pixel_values, output_hidden_states=True) # F x 574 x 1024
        ...
        image_features = self.multi_modal_projector(selected_image_feature) # F X H X W X C

        image_features = image_features.permute(0, 3, 1, 2).contiguous() # F X C X H X W
        
        image_features = (
            self.resampler(image_features)
            .flatten(2)
            .transpose(1, 2)
            .contiguous()
        ) # 
...
EntryClass = LlavaVidQwenForCausalLM
